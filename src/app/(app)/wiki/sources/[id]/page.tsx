...
export default async function SourcePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl p-5 md:p-8"><Link href="/wiki/sources" className="mb-5 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />{t("backToSources")}</Link>
    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_17rem]"><SourceForm documentTypes={documentTypes} redirectTo="/wiki/sources" initial={{ ...data.source, ...metadataDefaults, tags: data.tags }} /><aside className="space-y-6"><AttachmentPanel entityType="wikiSource" entityId={id} initial={attachments} />
