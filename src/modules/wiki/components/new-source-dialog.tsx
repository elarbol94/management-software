...
export function NewSourceDialog({ documentTypes = [], compactButton = false, redirectTo }: { documentTypes?: string[]; compactButton?: boolean; redirectTo?: string }) { const t = useTranslations("wiki"); const [open, setOpen] = useState(false); return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger render={<Button variant={compactButton ? "outline" : "default"} size={compactButton ? "sm" : "default"}><Plus className="size-4" />{t("newSource")}</Button>} /><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{t("newSource")}</DialogTitle></DialogHeader>{open && <SourceForm documentTypes={documentTypes} compact redirectTo={redirectTo} onSaved={() => setOpen(false)} />}</DialogContent></Dialog>; }
