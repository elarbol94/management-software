...
function AttachmentPanelContent({ entityType, entityId, initial, panelRef }: AttachmentPanelProps & { panelRef: ForwardedRef<AttachmentPanelHandle> }) {
  const t = useTranslations("wiki"); const common = useTranslations("common"); const input = useRef<HTMLInputElement>(null); const [files, setFiles] = useState(initial); const [pending, setPending] = useState(false); const [removingId, setRemovingId] = useState(""); const [error, setError] = useState("");
