...
export function SourceFilters({ initialQuery, initialStatus }: { initialQuery: string; initialStatus: string }) {
  ...
  return <div className="flex flex-wrap gap-2"><div className="relative min-w-64 flex-1"><Search className="absolute top-2.5 left-3 size-4 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") apply(); }} placeholder={t("searchSources")} className="pl-9" /></div><Select value={status} onValueChange={(value) => { const nextStatus = value ?? "all"; setStatus(nextStatus); apply(query, nextStatus); }}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{t("allReadingStatuses")}</SelectItem>{["toRead","reading","read"].map((item) => <SelectItem value={item} key={item}>{t(`readingStatuses.${item}`)}</SelectItem>)}</SelectContent></Select></div>;
}
