...
export function WikiShell({ page, backlinks, allPages, sources, citationSources, research, comments, users, attachments, meta }: {
  ...
  return <main className={isFocused ? "mx-auto max-w-[80rem] p-4 md:p-7" : "mx-auto max-w-[112rem] p-4 md:p-7"}>
    ...
    <div className={isFocused ? "mx-auto w-full max-w-[76rem]" : "grid gap-7 xl:grid-cols-[minmax(0,1fr)_17rem]"}>
      <section className="min-w-0">
        <WikiEditor key={page.id} focused={isFocused} pageId={page.id} pageVersion={page.version} initialContent={page.contentJson} allPages={allPages} sources={sources} users={users} citationLocale={citationLocale} comments={comments} pageActions={{ addAttachment: () => attachmentRef.current?.openFilePicker(), linkSupportingSource: () => { supportingSourceSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); setSupportingSourceOpen(true); requestAnimationFrame(() => supportingSourceTriggerRef.current?.focus()); } }} />
