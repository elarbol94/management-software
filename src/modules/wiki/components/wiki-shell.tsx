...
export function WikiShell({ page, backlinks, allPages, sources, citationSources, research, comments, currentUserId, users, attachments, documentTemplates, meta }: {
  ...
  return <main className={isFocused ? "w-full max-w-none p-4 md:p-7" : "mx-auto max-w-[112rem] p-4 md:p-7"}>
    ...
    <div className={isFocused ? "w-full" : "grid gap-7 xl:grid-cols-[minmax(0,1fr)_17rem]"}>
      <section className="min-w-0">
        <WikiEditor key={page.id} focused={isFocused} pageId={page.id} pageVersion={page.version} initialContent={page.contentJson} initialDocumentMode={page.documentMode} initialDocumentSettings={page.documentSettingsJson} documentTemplates={documentTemplates} allPages={allPages} sources={sources} users={users} citationLocale={citationLocale} comments={comments} currentUserId={currentUserId} pageActions={{ addAttachment: () => attachmentRef.current?.openFilePicker(), linkSupportingSource: () => { supportingSourceSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); setSupportingSourceOpen(true); requestAnimationFrame(() => supportingSourceTriggerRef.current?.focus()); } }} />
