...
export function WikiShell({ page, backlinks, allPages, sources, citationSources, research, comments, users, attachments, meta }: {
  ...
  return <main className="mx-auto max-w-[112rem] p-4 md:p-7">
    <header className="mb-5 border-b pb-4">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2"><Select value={status} onValueChange={(value) => { if (!value) return; const next = value as typeof status; setStatus(next); void saveMeta(next, citationLocale); }}><SelectTrigger className="h-8 w-36"><SelectValue /></SelectTrigger><SelectContent>{["inbox","working","evergreen"].map((item) => <SelectItem key={item} value={item}>{t(`pageStatuses.${item}`)}</SelectItem>)}</SelectContent></Select>
