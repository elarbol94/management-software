      ...
      {!isFocused && <aside data-testid="note-metadata-sidebar" className="space-y-6 xl:border-l xl:pl-6">
        <section data-testid="note-metadata-controls" className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <Select value={status} onValueChange={(value) => { if (!value) return; const next = value as typeof status; setStatus(next); void saveMeta(next, citationLocale); }}><SelectTrigger className="h-8 w-full"><SelectValue /></SelectTrigger><SelectContent>{["inbox","working","evergreen"].map((item) => <SelectItem key={item} value={item}>{t(`pageStatuses.${item}`)}</SelectItem>)}</SelectContent></Select>
            <Select value={citationLocale} onValueChange={(value) => { if (!value) return; setCitationLocale(value); void saveMeta(status, value); }}><SelectTrigger className="h-8 w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="de-DE">APA · DE</SelectItem><SelectItem value="en-US">APA · EN</SelectItem></SelectContent></Select>
          </div>
